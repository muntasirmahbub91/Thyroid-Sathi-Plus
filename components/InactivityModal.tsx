...
const InactivityModal: React.FC<InactivityModalProps> = ({ onClose }) => {
    const { copy } = useCopy();
    const { inactivityModal: modalCopy } = copy;
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-thy-surface rounded-3xl shadow-thy-emphasis max-w-sm w-full m-4 p-6 text-center" onClick={e => e.stopPropagation()}>
                <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-thy-accent-weak mb-4">
                    <ExclamationIcon className="h-6 w-6 text-thy-accent" />
                </div>
